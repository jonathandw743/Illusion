-- local mod_dir = require("lovely").mod_dir
-- print(mod_dir)
...
IllusionMod.image_data_or_path = function(asset)
    ...
    -- TODO: use errors provided
    local file_data = IllusionMod.nativefs.newFileData(path)
    if not file_data then
        print("nativefs.newFileData failed for " .. asset.name .. " at " .. path .. " falling back to " .. asset.path)
        return asset.path
    end
    -- TODO: use errors provided
    local image_data = love.image.newImageData(file_data)
    if not image_data then
        print("love.image.newImageData failed for " .. asset.name .. " at " .. path .. " falling back to " .. asset.path)
        return asset.path
    end
    return image_data
end
...
IllusionMod.shader_text_or_path = function(asset)
    local path = IllusionMod.mod_dir .. asset.path
    local shader_text = IllusionMod.nativefs.read(path)
    -- TODO: use errors provided
    if not shader_text then
        print("nativefs.read failed for " .. asset.name .. " at " .. path .. " falling back to " .. asset.path)
        return asset.path
    end
    return shader_text
end

-- print(IllusionMod.mod_dir)
-- IllusionMod.load_shaders = function()
--     IllusionMod.SHADERS = {}
--     for name in IllusionMod.nativefs.getDirectoryItems(lovely.mod_dir.."/Illusion/assets/shaders") do
--         local extension = string.sub(name, -3)
--         if extension == '.fs' then
--             local shader_name = string.sub(name, 1, -4)
--             IllusionMod.SHADERS[shader_name] = love.graphics.newShader(IllusionMod.nativefs.read(lovely.mod_dir.."/Illusion/assets/shaders/"..name))
--         end
--     end
-- end
-- IllusionMod.
...
--     IllusionMod.SHADERS["dissolve"] = love.graphics.newShader [[
